...
- transitional public API 和需要外迁/删除的主仓 adapter/template 已被 `v1-migration-plan.json` 覆盖，且该计划的 `release_gate_checks` 已把每个 v1 gate 绑定到 evidence type、来源 manifest、required status 和 blocker summary；
