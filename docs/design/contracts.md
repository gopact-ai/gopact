...
`Artifact`、`Policy` 和 `LeaseBackend` 是基础契约。它们有默认实现和可替换 adapter，但不归入 provider/tool/sandbox/memory/skill/MCP/A2A 这类业务运行时模块。
