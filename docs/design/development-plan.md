...
- `[done: seventh slice]` workflow process conformance 已进一步固定 workflow output 的 input/intervention 总计数，以及每个 child action summary 的 input/intervention 局部计数，避免外部 template 在重排或裁剪 process records 时留下不可恢复的摘要漂移。
...
- `[done: third slice]` workflow process conformance：`templates/devagent.CheckWorkflowProcessConformance` / `RequireWorkflowProcessConformance` 可供外部 Dev Agent template 复用，固定 workflow process records 的父子 task 链接、action 顺序、失败摘要、required input source、workflow output 与 child summary 的 input/intervention count、input/intervention workflow metadata 和 patch input raw diff 不泄漏；
