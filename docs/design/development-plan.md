...
- `[done: third slice]` workflow process conformance：`templates/devagent.CheckWorkflowProcessConformance` / `RequireWorkflowProcessConformance` 可供外部 Dev Agent template 复用，固定 workflow process records 的父子 task 链接、child task input/output 的 mode/action/status、action 顺序、失败摘要、required input source、workflow output 与 child summary 的 mode/action/action_status/reason_count 以及 input/intervention count、input/intervention workflow metadata、合法 action index、boundary mode/action/action_status 对齐、release action 的 gate/review boundary 对账和 patch input raw diff 不泄漏；
