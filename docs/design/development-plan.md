...
- `[done: third slice]` workflow process conformance：`templates/devagent.CheckWorkflowProcessConformance` / `RequireWorkflowProcessConformance` 可供外部 Dev Agent template 复用，固定 workflow process records 的父子 task 链接、action 顺序、失败摘要、required input source、workflow output 与 child summary 的 mode/action/action_status 以及 input/intervention count、input/intervention workflow metadata、合法 action index、boundary mode/action/action_status 对齐和 patch input raw diff 不泄漏；
