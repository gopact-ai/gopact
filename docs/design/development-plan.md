...
- `[done: third slice]` workflow process conformance：`templates/devagent.CheckWorkflowProcessConformance` / `RequireWorkflowProcessConformance` 可供外部 Dev Agent template 复用，固定 workflow process records 的父子 task 链接、action 顺序、失败摘要、required input source、input/intervention workflow metadata 和 patch input raw diff 不泄漏；
