...
Channel adapter 同理不应该成为另一套 runtime。A2UI、AG-UI、SSE、TUI、Lark bot、飞书卡片等 adapter 只消费 event stream、`SurfaceMessage`、artifact refs 和 interrupt/resume 事件。Root package 已提供 `Transfer`、`Channel`、`ChannelEvent` 和 `ChannelEvent.ResumeRequest()` 的基础契约；`adapters/channel/tui` 已落地 writer-based TUI 第一片，`adapters/channel/sse` 已落地 HTTP SSE 第一片，`adapters/channel/lark` 已落地 host-injected Lark text/interactive payload、sender、callback source 和 action value decode 第一片，`adapters/channel/a2ui` 已落地 A2UI v0.9 JSON message transfer、JSONL channel、history replay、schema catalog validation、component JSON Schema validator 注入、client-supported catalog negotiation、in-memory reference renderer 和 action decode 第一片，`adapters/channel/agui` 已落地 AG-UI event transfer、HTTP SSE event stream channel 和 action POST 回流第一片。生产级 adapter 负责把统一 `SurfaceMessage` 转成目标 payload、投递 payload、接收用户动作和维护目标平台 message id。用户动作必须通过 TurnLoop 的 input/resume/action 边界回到运行时，不能从 channel adapter 直接调用 graph/node。
