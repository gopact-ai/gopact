...
- M5 Agent-as-Tool authenticated cancel trajectory：`templates/agenttool` 已通过 `a2a_auth_cancel.golden.json` 固定 remote A2A authenticated cancel 的 policy requested/decided -> canceled 事件顺序，并在测试中校验 sanitized auth context 贯穿 policy、remote cancel context、terminal event 和 tool result metadata。
