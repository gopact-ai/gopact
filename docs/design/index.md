...
| `cmd/gopact-extscaffold` | 外部仓库 scaffold workspace 生成命令，支持 `go run ./cmd/gopact-extscaffold -root . -out <dir> -verify`、`-dry-run`、`-plan-json`、`-plan-sh`、`-remote-status-json`、本地 `go.work` bootstrap 和输出目录 `sync-plan.json` / `sync-repos.sh` | 真实创建或同步 gopact-* 私有仓库前，先生成可复制/提交的本地 scaffold workspace、逐仓库跑 generated repo conformance，导出远端 bootstrap/sync JSON 计划或 shell 脚本；同步脚本会用 `GOWORK=off go mod tidy` 生成远端可用 `go.sum`，并只读审计远端仓库、CI workflow 与最新 Actions run 状态 |
