...
| [../../FEATURES.md](../../FEATURES.md) | core SDK 可执行能力覆盖矩阵，绑定每个核心能力的离线测试或 conformance 入口 | 判断功能点是否已有可运行验收，避免文档能力与测试脱节 |
...
| `cmd/gopact-extscaffold` | legacy 外部仓库 scaffold workspace 生成命令，支持 `go run ./cmd/gopact-extscaffold -root . -out <dir> -verify`、`-dry-run`、`-plan-json`、`-plan-sh`、`-plan-secrets-sh`、`-plan-rerun-sh`、`-remote-status-json`、`-remote-status-evidence-json`、`-remote-ci-evidence-json`、本地 `go.work` bootstrap 和输出目录 `sync-plan.json` / `sync-repos.sh` / `sync-secrets.sh` / `rerun-ci.sh` | 复核旧 scaffold workspace、远端 readiness audit 和 release gate evidence；当前新增官方 extension 进入 `gopact-ext` |
