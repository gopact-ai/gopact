...
- M5 template trajectory / workflow evidence：`templates/agenttool` 已通过 local child run、local child failure、remote A2A stream、remote A2A send failure、remote A2A send timeout、remote A2A send policy deny、remote A2A stream policy deny、remote A2A cancel success 和 remote A2A cancel policy deny golden trajectory 固定 child event sequence；`templates/devagent.BuildWorkflowProcessRecords` 已覆盖 rejected child action summary，workflow 父 task output 会保留 child `action_status`、`reason_count`、`failed_action_count` 和 sanitized child summary，patch input 不泄露 raw diff；`templates/devagent` 已用 self-bootstrap release golden trajectory 固定 run_started -> analyze -> plan -> release_gate -> run_completed 事件序列，用 self-bootstrap apply release golden trajectory 固定 run_started -> analyze -> plan -> apply_patch -> release_gate -> run_completed，policy/sandbox 事件由 action gate 固定，diff/checkpoint/trajectory evidence 进入 release gate，用 rejected apply golden trajectory 固定缺少 policy/sandbox/diff/checkpoint evidence 的 run_started -> analyze -> plan -> apply_patch failed -> run_failed，并用 rejected release golden trajectory 固定 run_started -> analyze -> plan -> release_gate failed -> run_failed 事件序列，同时把 `trajectory_golden` / failed release gate evidence 纳入对应 release gate / failed report 路径；rejected release 真实场景还会把 workflow process records 写入 `RunExport`，并从 export 反取后复用 workflow process conformance 校验。
- M5 workflow export/import helper：`templates/devagent.WorkflowRecordsFromRunExport` 可从 `RunExport` 恢复 Dev Agent workflow parent/child/input/intervention process records，默认使用 `devagent:<runID>:workflow`，要求至少存在一个 child task，按 `workflow_action_index` 恢复顺序并返回 defensive copy；它只恢复已观察过程证据，不调度 workflow、不重新执行 action。
- M5 workflow process conformance：`templates/devagent.CheckWorkflowProcessConformance` / `RequireWorkflowProcessConformance` 已固定 workflow parent task、workflow parent input action_count、child task parent link、child task input/output 的 mode/action/status/reason_count、action order、failed action summary、required input source、workflow output 与 child summary 的 mode/action/action_status/reason_count 以及 input/intervention count、release child summary 的 gate/review boundary id、input/intervention workflow metadata、合法 action index、boundary mode/action/action_status 对齐、release gate input 对账、approved/rejected review intervention 对账和 patch input raw diff 不泄漏，外部 Dev Agent template 可复用该 helper 检查已观察 process records。
