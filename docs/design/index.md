...
- M5 workflow process conformance：`templates/devagent.CheckWorkflowProcessConformance` / `RequireWorkflowProcessConformance` 已固定 workflow parent task、child task parent link、action order、failed action summary、required input source、workflow output 与 child summary 的 input/intervention count、input/intervention workflow metadata 和 patch input raw diff 不泄漏，外部 Dev Agent template 可复用该 helper 检查已观察 process records。
