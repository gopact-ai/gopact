...
- M5 Agent-as-Tool approval boundary：A2A send/stream/cancel policy review 都会在远程动作执行前返回 approval interrupt；approval interrupt id 会携带 `:a2a:<action>`，避免同一 parent/child call 下 send/cancel review 的 resume id 冲突；A2A auth 会在 send/stream/cancel 的 policy 和 remote action 之前注入，policy requested/decided event、send completed/failed event、cancel canceled/failed event 与 tool result metadata 都会保留 sanitized auth audit 字段。
