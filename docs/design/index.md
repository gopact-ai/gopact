...
- M5 Agent-as-Tool approval boundary：A2A send/stream/cancel policy review 都会在远程动作执行前返回 approval interrupt；approval interrupt id 会携带 `:a2a:<action>`，避免同一 parent/child call 下 send/cancel review 的 resume id 冲突。
