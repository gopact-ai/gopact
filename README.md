...
- `tools`：visible/deferred tool registry、tool search、promotion、direct invocation、model-visible invocation guard、tool middleware、scope metadata 进入 tool policy、tool-call `EffectRecord` 第一片、`ToolResult.Commit` 幂等提交契约第一片、`CommitStore` replay commit ledger 插槽、内存 commit store 参考实现、tool-level retry policy contract / middleware、tool-call replay handler 第一片、effect graph/replay policy 契约、`PlanEffectReplay` 单步 runtime plan、`BuildRunEffectGraph` 跨 step effect graph、`PlanRunEffectReplay` run-level plan、`EffectReplayRegistry` handler 分发，`ExecuteEffectReplay` / `ExecuteRunEffectReplay` executor 第一片，以及 step-level/run-level effect replay verification evidence 桥接。
...
- `docs/design/v1-migration-plan.json`：v1 前 core 边界收敛计划，把需要外迁的主仓路径和 transitional root API 分别绑定到目标仓库、目标状态和 `release_gate_checks`；每个 gate 同时声明 evidence type、来源 manifest、`required_check_ids`，CI 相关 gate 还声明 `required_ci_gates`（核心 CI 对齐 `core-ci-gates.json`，外部仓 readiness 对齐 `external-ci:gopact-ai` 跨仓 CI gate），并由测试证明可直接转成 `gopacttest.VerificationEvidenceRequirement`、`templates/devagent.Require*` option 和 `ReleaseBundle` requirement，让 v1 release gate 能消费同一份机器可读证据要求。
...
- `internal/extensionscaffold`：本仓内部外部仓库 scaffold materializer，`LoadRepositoriesFromDesign` 可从 `docs/design/external-repositories.json`、`docs/design/extension-conformance.json`、`docs/design/extension-scaffold-spec.json` 和 `docs/design/v1-migration-plan.json` 读取仓库计划与 v1 迁移责任，`WriteRepositoriesFromDesign` 可把所有计划批量写成 `<output>/<repo-name>/go.mod`、README、CONFORMANCE、CI 和 `examples/minimal_test.go`，并在 README/CONFORMANCE 中渲染 `V1 Migration Ownership`、在 CONFORMANCE 中记录已知 gopacttest helper reference、由 minimal test 做机器检查，`RenderSyncPlanFromDesign` 可生成包含私有仓库创建命令、文件清单、CI 命令和本地验证命令的机器可读远端同步计划，`RenderSyncScriptFromDesign` / `RenderSecretScriptFromDesign` / `RenderRerunScriptFromDesign` 可生成可审查的 GitHub 私有仓库同步、secret 配置和 CI rerun 脚本，`CheckRemoteRepositories` 可生成远端 readiness 审计报告，`RecordRemoteStatusCheck` 可把已观察 `RemoteStatusReport` 转成标准 `external_repository_readiness` verification evidence，`RecordRemoteCIRunSetCheck` 可把同一报告中的最新 Actions run 转成 `external-ci:gopact-ai` 跨仓 `ci_gate` verification evidence；它不属于 root SDK API，也不读取 SDK 运行时配置文件。
