...
- `WorkflowActionProcessRecordsFromRunExport`：从已落盘 `RunExport` 恢复 Dev Agent workflow 并直接提取单个 child action 的 process records，适合外部 template 做 step 级 release/apply/plan export/import；它只恢复、校验和提取已观察过程证据，不调度 workflow、不重新执行 action。
