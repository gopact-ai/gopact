...
- `WorkflowActionProcessRecordsByTaskID` / `WorkflowActionProcessRecordsFromRunExportByTaskID`：可按 child task id 提取单个 action 的 process records；`WorkflowActionProcessRecordsFromRunExport` 则按 1-based action index 从已落盘 `RunExport` 恢复并提取。它们适合外部 template 做 step 级 release/apply/plan export/import，只恢复、校验和提取已观察过程证据，不调度 workflow、不重新执行 action。
