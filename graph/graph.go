...
func (r *Runnable[S]) execute(ctx context.Context, initial S, yield func(gopact.Event, error) bool, opts ...InvokeOption) (S, error) {
	...
	if cfg.stepExport != nil {
		resumedState, resumedQueue, resumedStep, resumedIDs, err := r.resumeFromStepExport(ctx, *cfg.stepExport, cfg.resumeRequest, cfg.schemaValidator)
		if err != nil {
		...
		if ok {
			resumedState, resumedQueue, resumedStep, resumedIDs, err := r.resumeFromCheckpoint(ctx, checkpoint, cfg.resumeRequest, cfg.schemaValidator)
			if err != nil {
				var interruptErr *gopact.InterruptError
	...
	for len(queue) > 0 {
		...
		nodeCtx := gopact.NewNodeContext(ctx, gopact.NodeContextOptions{
			IDs:   cfg.ids,
			Node:  name,
			Step:  nextStep,
			Input: state,
		...
		if err != nil {
			...
			if errors.As(err, &interruptErr) {
				...
				interruptedSnapshot.Queue = append([]string(nil), r.edges[name]...)
				if checkpointer != nil {
			...
			if errors.Is(err, context.Canceled) {
				...
				canceledSnapshot.Queue = append([]string(nil), r.edges[name]...)
				if checkpointer != nil {
		...
		nextQueue = append(nextQueue, r.edges[name]...)
		if checkpointer != nil {
...
func (r *Runnable[S]) resumeFromStepExport(ctx context.Context, export gopact.StepExport, resume *gopact.ResumeRequest, validator gopact.JSONSchemaValidator) (S, []string, int, gopact.RuntimeIDs, error) {
	var zero S
	if err := export.Validate(); err != nil {
		return zero, nil, 0, gopact.RuntimeIDs{}, fmt.Errorf("graph: resume step export: %w", err)
	}
	if export.Step.Phase != gopact.StepCompleted && export.Step.Phase != gopact.StepInterrupted && export.Step.Phase != gopact.StepCanceled {
		return zero, nil, 0, gopact.RuntimeIDs{}, fmt.Errorf("graph: resume step phase %q is not supported", export.Step.Phase)
	}
	if export.Step.Phase == gopact.StepInterrupted {
		if resume == nil {
			return zero, nil, 0, gopact.RuntimeIDs{}, &gopact.InterruptError{Record: *export.Step.Pending}
		}
		if err := resume.Validate(); err != nil {
			return zero, nil, 0, gopact.RuntimeIDs{}, fmt.Errorf("graph: resume request: %w", err)
		}
		if resume.InterruptID != export.Step.Pending.ID {
			return zero, nil, 0, gopact.RuntimeIDs{}, fmt.Errorf("graph: resume interrupt id %q does not match pending interrupt %q", resume.InterruptID, export.Step.Pending.ID)
		}
		if resume.StepID != "" && resume.StepID != export.Step.ID {
			return zero, nil, 0, gopact.RuntimeIDs{}, fmt.Errorf("graph: resume step id %q does not match export step %q", resume.StepID, export.Step.ID)
		}
		if err := gopact.ValidateResumePayloadWithValidator(ctx, validator, *export.Step.Pending, *resume); err != nil {
			return zero, nil, 0, gopact.RuntimeIDs{}, fmt.Errorf("graph: resume payload: %w", err)
		}
	}
	if export.Step.Node != Start && export.Step.Node != End {
		if _, ok := r.nodes[export.Step.Node]; !ok {
			return zero, nil, 0, gopact.RuntimeIDs{}, fmt.Errorf("graph: resume node %q is missing", export.Step.Node)
		}
	}
	state, ok := export.Step.Output.(S)
	if !ok {
		return zero, nil, 0, gopact.RuntimeIDs{}, fmt.Errorf("graph: resume state type mismatch: got %T", export.Step.Output)
	}
	queue := append([]string(nil), export.Step.Queue...)
	if len(queue) == 0 {
		queue = append(queue, r.edges[export.Step.Node]...)
	}
	return state, queue, export.Step.Step, export.Step.IDs, nil
}

func (r *Runnable[S]) resumeFromCheckpoint(ctx context.Context, checkpoint Checkpoint[S], resume *gopact.ResumeRequest, validator gopact.JSONSchemaValidator) (S, []string, int, gopact.RuntimeIDs, error) {
	var zero S
	if checkpoint.Step < 0 {
		return zero, nil, 0, gopact.RuntimeIDs{}, fmt.Errorf("graph: checkpoint step must be non-negative, got %d", checkpoint.Step)
	}
	if checkpoint.Node == "" {
		return zero, nil, 0, gopact.RuntimeIDs{}, errors.New("graph: checkpoint node is required")
	}

	phase := checkpointPhase(checkpoint)
	if phase != gopact.StepCompleted && phase != gopact.StepInterrupted && phase != gopact.StepCanceled {
		return zero, nil, 0, gopact.RuntimeIDs{}, fmt.Errorf("graph: checkpoint phase %q is not supported", phase)
	}
	if phase == gopact.StepInterrupted {
		if checkpoint.Pending == nil {
			return zero, nil, 0, gopact.RuntimeIDs{}, errors.New("graph: interrupted checkpoint pending record is required")
		}
		if err := checkpoint.Pending.Validate(); err != nil {
			return zero, nil, 0, gopact.RuntimeIDs{}, fmt.Errorf("graph: interrupted checkpoint pending record: %w", err)
		}
		if resume == nil {
			return zero, nil, 0, gopact.RuntimeIDs{}, &gopact.InterruptError{Record: *checkpoint.Pending}
		}
		if err := resume.Validate(); err != nil {
			return zero, nil, 0, gopact.RuntimeIDs{}, fmt.Errorf("graph: resume request: %w", err)
		}
		if resume.CheckpointID != "" && checkpoint.ID != "" && resume.CheckpointID != checkpoint.ID {
			return zero, nil, 0, gopact.RuntimeIDs{}, fmt.Errorf("graph: resume checkpoint id %q does not match checkpoint %q", resume.CheckpointID, checkpoint.ID)
		}
		if resume.InterruptID != checkpoint.Pending.ID {
			return zero, nil, 0, gopact.RuntimeIDs{}, fmt.Errorf("graph: resume interrupt id %q does not match pending interrupt %q", resume.InterruptID, checkpoint.Pending.ID)
		}
		ids := checkpointIDs(checkpoint)
		if resume.StepID != "" && resume.StepID != stepID(ids, checkpoint.Step) && resume.StepID != checkpoint.ID {
			return zero, nil, 0, gopact.RuntimeIDs{}, fmt.Errorf("graph: resume step id %q does not match checkpoint step %q", resume.StepID, stepID(ids, checkpoint.Step))
		}
	...
	if checkpoint.Node != Start && checkpoint.Node != End {
		if _, ok := r.nodes[checkpoint.Node]; !ok {
			return zero, nil, 0, gopact.RuntimeIDs{}, fmt.Errorf("graph: checkpoint node %q is missing", checkpoint.Node)
		}
	}
	...
	return checkpoint.State, queue, checkpoint.Step, checkpointIDs(checkpoint), nil
}
