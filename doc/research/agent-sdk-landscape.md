...
项目级设计哲学已经沉淀到 `doc/design/philosophy.md`。本文解释这些原则从哪里来；哲学文档是后续 API 和运行时工作的决策指南。
...
- `adapters/channel/a2ui` 已有第一片：把 `SurfaceMessage` 转成 A2UI messages，并通过 JSONL channel 投递，成功投递的 message snapshot 可用于 history replay，也可通过本地 catalog registry 做组件名、child 引用和 component JSON Schema subset 校验；当前 subset 覆盖常见 object/string/number/array 约束和 `items` 递归校验。transfer 可按 host 注入的 renderer supported catalog ID 顺序选择首个匹配 catalog，未命中时回退到显式配置或默认 catalog。生产级 renderer、完整 JSON Schema engine 和更完整 catalog negotiation 深化仍后置。
...
AG-UI 是面向 agent 前端集成的事件协议。官方架构把它定义成 lightweight、event-driven、bidirectional、transport-agnostic 的 agent/user app 通信层；核心抽象是一次 `run(input)` 返回一串 `BaseEvent`，具体 transport 可以是 SSE、WebSocket、webhook 或自定义 HTTP。

值得吸收的想法：

- 前端可观测和交互应该先表现为结构化 event stream，而不是绑定某个 WebSocket server。
- AG-UI event 分层清晰：run lifecycle、text message、tool call、state、activity、custom/raw、reasoning 等类别可以映射到 `gopact.Event` 和 `SurfaceMessage`。
- Text message 是 start/content/end 三段式；流式 token 更适合映射为 `TEXT_MESSAGE_CONTENT`，不应该每个 delta 都伪造成完整 run。
- `CUSTOM` event 为 SDK 自有 surface 语义提供了逃生口，例如 approval、selection、artifact summary 等暂未有 AG-UI 原生事件的场景。

对 `gopact` 的含义：

- AG-UI 不进入 core runtime；它属于 `adapters/channel/agui` 或 channel plugin。
- `adapters/channel/agui` 已落地 event transfer 与 HTTP SSE channel 第一片：把 `SurfaceMessage` 转成 `RUN_STARTED`、`TEXT_MESSAGE_*`、`RUN_ERROR`、`CUSTOM`、`RUN_FINISHED` 等 AG-UI event sequence；`text_delta` 只投影为 `TEXT_MESSAGE_CONTENT`，`StreamHandler` 逐帧广播 AG-UI event，`ActionHandler` 把 HTTP action POST 转回 `ChannelEvent`。
- AG-UI WebSocket、plugin 化、鉴权、session replay 和生产级用户映射仍应由宿主注入或后续 plugin 提供，SDK 不读取配置文件、不持有 endpoint/auth secret。
...
完整项目哲学见 `doc/design/philosophy.md`。简版如下：
...
调研初稿阶段，当前实现尚不具备插件能力或中间件能力。后续代码已经落地第一批切片：`graph.Run` event stream、root `Runner`、`NodeContext`、`EventContext`、`PluginHost`、`PluginDescriptor` / `PluginCapability` 能力声明、插件 subscriber strict/fallback、`AsyncEventSink` bounded queue/backpressure、接入 `graph.WithNodeMiddleware` 的 Gin 风格 node middleware、接入 Runner 的 event middleware、接入 provider router 的 model middleware、接入 tools registry 的 tool middleware、model/tool policy middleware、memory/sandbox/artifact/channel policy wrapper、skill registry/resource/script policy wrapper、trace exporter policy wrapper、MCP manager/client policy wrapper、event redaction middleware，以及 plugin lifecycle 状态机、幂等 close 和 close-while-running 等待。

完整扩展性设计见 `doc/design/extensibility.md`。该设计已经明确：
...
- MCP overview：https://modelcontextprotocol.io/doc/getting-started/intro
- MCP architecture：https://modelcontextprotocol.io/doc/learn/architecture
- A2A specification：https://github.com/a2aproject/A2A/blob/main/doc/specification.md
...
- OpenRouter model fallbacks：https://openrouter.ai/doc/guides/routing/model-fallbacks
- OpenRouter provider routing：https://openrouter.ai/doc/guides/routing/provider-selection
- OpenRouter auto router：https://openrouter.ai/doc/guides/routing/routers/auto-router
- CC Switch：https://github.com/farion1231/cc-switch
- oh-my-pi model configuration：https://github.com/can1357/oh-my-pi/blob/main/doc/models.md
- Eino ADK 概览：https://www.cloudwego.io/doc/eino/core_modules/eino_adk/
- Eino v0.9 agentic-runtime：https://www.cloudwego.io/zh/doc/eino/release_notes_and_migration/eino_v0.9._agentic-runtime/
- Eino ADK agent 概览：https://www.cloudwego.io/doc/eino/core_modules/eino_adk/agent_preview/
- Eino ADK quickstart：https://www.cloudwego.io/doc/eino/core_modules/eino_adk/agent_quickstart/
- Eino components：https://www.cloudwego.io/doc/eino/core_modules/components/
- Eino graph 介绍：https://www.cloudwego.io/doc/eino/core_modules/chain_and_graph_orchestration/chain_graph_introduction/
- Eino workflow：https://www.cloudwego.io/doc/eino/core_modules/chain_and_graph_orchestration/workflow_orchestration_framework/
- Eino tools：https://www.cloudwego.io/doc/eino/core_modules/components/tools_node_guide/
...
Research notes comparing LangGraph, LangChain, Eino, Google ADK, A2UI, OpenRouter, and related systems. It informs design choices but is not an API contract.
