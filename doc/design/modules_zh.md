...
- `a2a.HTTPRegistry`、`a2a.NewHTTPRegistryHandler`、`a2a.Registry.RegisterCardWithLease` 和 `a2a.Registry.HeartbeatCard` 已提供 HTTP agent card lease registration / heartbeat 第一片；远程 agent 可把自身 card 按 TTL 注册到轻量 HTTP registry，registry 只持有可发现 card，不持有远程进程；`WithHTTPReadinessCheck` 可让 HTTP registry 在 `ListCards` / `Discover` / `Mesh.Bootstrap` 时过滤未 ready 的 card，并按 card URL 路由到 HTTP agent；
...
- `a2a.Mesh.SyncEvery` 已提供持续 discovery sync 第一片；宿主可以用 `context.Context` 控制生命周期，立即执行一次 sync，并按 interval 重复执行。
