...
`gopact-ext/devagent/selfbootstrap` 提供第一段 provider-neutral 的 Dev Agent 自举 workflow。它编排由宿主注入的 analyze、plan、patch proposal policy、write、test、review 阶段，封存已观察到的 policy decision、diff、file snapshot、command、CI gate、review、run export、failure attribution 和 verification report 证据，并登记在 [workflow-orchestration-matrix.json](workflow-orchestration-matrix.json)。

`gopact-ext/devagent/workspace` 提供第一段本地仓库 adapter。它通过显式 `PlanPatchWriter` 应用 policy-approved plan patch，也支持通过 `PatchWriter` 应用调用方提供的 unified diff，在 apply 前拒绝越过 root 的路径和 symlink escape，采集 repo-relative git diff 和 file snapshot，把宿主控制的本地命令执行结果记录为有界 command 与 CI gate evidence，把失败命令保留为 verification evidence，并由 `gopact-examples` 的 self-bootstrap quickstart 在临时 git workspace 中演示。
...
- `gopact agent init <name> [-module <module>] [-out <dir>]` 创建独立的 A2A HTTP agent module，包含测试、health/readiness endpoint、`agents.json` bare-array registry、`.env.example` 和本地 README；未传 `-module` 时默认使用 `example.com/<name>`。
- `gopact agent init-cluster <name> [-module <module>] [-out <dir>] [-agent name[:capability[:description]]]...` 创建本地 A2A HTTP cluster，默认包含 planner/worker/reviewer，也可通过自定义 domain-agent specs 生成垂域 agent，包含 HTTP registry bootstrap、mesh routing、streaming、cancel、health/readiness、graceful shutdown 和 bare-array `agents.json` 测试；未传 `-module` 时默认使用 `example.com/<name>`。
