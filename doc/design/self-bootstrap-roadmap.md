...
- `gopact agent init-cluster <name> [-module <module>] [-out <dir>] [-agent name[:capability[:description]]]...` creates a local A2A HTTP cluster with default planner/worker/reviewer agents or custom domain-agent specs, plus tests for HTTP registry bootstrap, mesh routing, streaming, cancel, health/readiness, graceful shutdown, and a bare-array `agents.json`. When `-module` is omitted, the scaffold uses `example.com/<name>`.
