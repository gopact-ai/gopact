...
本文定义 `gopact` 的 hook、middleware、plugin 扩展模型。当前代码已经实现第一批切片：root `NodeContext`、`EventContext`、`ModelContext`、`ToolContext`、`PluginHost`、`SurfaceMessage`、`Transfer`、`Channel`、`ChannelEvent`、`PolicyChannel`、writer-based TUI transfer/channel adapter 第一片、HTTP SSE transfer/channel adapter 第一片、host-injected Lark transfer/channel/callback source 第一片、A2UI v0.9 JSON message transfer/JSONL channel/history replay/schema catalog validation/component JSON Schema validator 注入/client-supported catalog negotiation/in-memory reference renderer/action decode 第一片、AG-UI event transfer/HTTP SSE channel 第一片、skill filesystem registry loader、skill registry/resource/script policy wrapper、skill local resource reader、sandbox script runner、trace HTTP/JSON exporter、trace OTLP/HTTP JSON exporter、LangSmith-compatible HTTP run exporter、LangGraph-style HTTP event exporter、trace exporter policy wrapper、MCP newline JSON-RPC client/transport、MCP newline interleaved server-to-client request handler、MCP newline notification handler、MCP Streamable HTTP POST + JSON/SSE response transport、MCP Streamable HTTP GET listen stream、MCP HTTP/SSE interleaved server-to-client capability request dispatch、MCP HTTP/SSE notification handler、MCP URL-mode elicitation completion notification handler 第一片、continuous listen reconnect/retry、session/protocol header、DELETE session termination 与 404 session-expired 处理第一片、MCP sampling/elicitation handler contract、policy wrapper 与 `CapabilityServer` JSON-RPC dispatch 第一片、MCP `ToolServer` server adapter、MCP manager/client policy wrapper、事件订阅、接入 `graph.WithNodeMiddleware` 的 node middleware、接入 Runner 的 event middleware、`EventSinkMiddleware` strict/fallback 策略、`AsyncEventSink` bounded queue/backpressure 第一片、接入 provider router 的 model middleware、接入 tools registry 的 tool middleware、model/tool/channel policy middleware、model I/O redaction middleware、model rate limit middleware、tool result redaction middleware、memory/sandbox/artifact policy wrapper、policy requested/decided events、review-to-approval interrupt、event redaction middleware、`Event.Redaction` 状态，以及 Runner/TurnLoop close 第一版。model/tool middleware 已支持通过 `PluginHost` 统一注册；plugin lifecycle 状态机、幂等 close、close-while-running 等待、`PluginDescriptor` / `PluginCapability` 能力声明和 subscriber strict/fallback failure policy 已完成第一片。approval UI/channel、完整前端 A2UI renderer、完整 JSON Schema engine adapter/plugin 与 conformance 深化、更完整 catalog negotiation 深化、AG-UI WebSocket/plugin 等生产级 channel adapter、Lark 真实 client/OAuth/plugin 接入、真实 LangSmith Go SDK / dataset/evaluation/feedback/run query 和 LangGraph 专项 exporter 的 policy/redaction 深化仍属于后续设计目标。
...
- A2UI / AG-UI transfer/channel plugin；
...
- response validation；
- model I/O redaction：root `RedactModelRequest` / `RedactModelResponse` / `ModelIORedactionMiddleware` 可在 `c.Next()` 前后脱敏 model request/response；
- model rate limit：root `ModelRateLimiter` / `ModelRateLimiterFunc` / `ModelRateLimitMiddleware` 可在 `c.Next()` 前等待宿主注入 limiter；SDK 不内置限流算法、不读取配置；
...
- human approval；
- timeout；
- retry：root `ToolRetryPolicy` / `ToolRetryDecider` / `ToolRetryMiddleware` 可在 tool middleware 边界按显式 decision 重跑下游 handler，默认不重复执行无 idempotency key 的 tool；
- result redaction：root `RedactToolResult` / `ToolResultRedactionMiddleware` 可在 `c.Next()` 后脱敏 tool result；
- audit log；
- idempotency key。
...
- event sink 失败的策略必须可配置；当前 root `EventSinkMiddleware` 默认 `strict`，`WithEventSinkFallback()` 会把错误写入 `event_sink_error` metadata 并继续发出事件；
...
- subscriber 默认采用 strict 失败语义；`WithPluginFailureFallback()` 会把非致命 subscriber 错误写入事件 metadata 的 `plugin_subscriber_errors` 并继续发布。
...
1. `trace` plugin：把 `gopact` event stream 和 model/tool/node middleware 转成 provider-neutral `SpanRecord`，当前已有 HTTP/JSON exporter、OTLP/HTTP JSON exporter、LangSmith-compatible HTTP run exporter 和 LangGraph-style HTTP event exporter 第一片，可先发到宿主 collector、OpenTelemetry collector、LangSmith-compatible endpoint、LangGraph-style collector 或 vendor 网关。
2. `langsmith` plugin：复用 `trace` plugin 的 span 输出，后续通过 LangSmith Go SDK 或 LangSmith 专项 API 补齐 run 查询、dataset、evaluation、feedback 等更厚能力。
...
- `langsmith` plugin 应通过 `PluginDescriber` 声明 `telemetry` 能力，Host 会补齐其实际注册的 event/model/tool middleware 或 subscriber 能力；
- 不能改变 graph 分支、checkpoint 内容或业务结果；
- exporter 可以复用 `AsyncEventSink` 获得 bounded queue、block/drop-newest 背压和 close drain 语义；当前 HTTP/JSON exporter 已覆盖最小网络外发，OTLP/HTTP JSON exporter 已覆盖标准 OpenTelemetry collector 外发，LangSmith-compatible HTTP exporter 已覆盖 run payload 外发第一片，LangGraph-style HTTP exporter 已覆盖 thread/run/attempt event envelope 外发第一片，真实 LangSmith SDK exporter 后续接入；
- strict 模式可以让关键 trace 写入失败返回错误；普通 trace/exporter 建议显式使用 fallback；
...
```text
NewPluginHost / Install
  -> plugin Setup
NewRunner
  -> Run start
  -> plugin host active run acquire
  -> event stream
  -> Run end / Run error
  -> plugin host active run release
  -> runner Close
  -> wait active runs
  -> plugin Close
```
...
6. retry middleware 应包住 provider call；tool retry 必须通过 `ToolRetryDecider` / `DecideToolRetry` / `ToolRetryMiddleware` 消费 root retry decision contract，默认不重复执行非幂等 tool，除非宿主显式放开。
...
- event sink error 默认不应破坏业务执行，除非该 sink 被配置为 strict；root `EventSinkMiddleware` 的默认值是 strict，所以非关键 exporter 必须显式设置 fallback；
- plugin subscriber error 默认 strict；`WithPluginFailureFallback()` 只影响 subscriber publish，不改变 plugin setup/close 的严格失败语义；
...
Channel adapter 同理不应该成为另一套 runtime。A2UI、AG-UI、SSE、TUI、Lark bot、飞书卡片等 adapter 只消费 event stream、`SurfaceMessage`、artifact refs 和 interrupt/resume 事件。Root package 已提供 `Transfer`、`Channel`、`ChannelEvent` 和 `ChannelEvent.ResumeRequest()` 的基础契约；`adapters/channel/tui` 已落地 writer-based TUI 第一片，`adapters/channel/sse` 已落地 HTTP SSE 第一片，`github.com/gopact-ai/gopact-adapters-channel/lark` 已落地 host-injected Lark text/interactive payload、sender、callback source 和 action value decode 第一片，`github.com/gopact-ai/gopact-adapters-channel/a2ui` 已落地 A2UI v0.9 JSON message transfer、JSONL channel、history replay、schema catalog validation、component JSON Schema validator 注入、client-supported catalog negotiation、in-memory reference renderer 和 action decode 第一片，`github.com/gopact-ai/gopact-adapters-channel/agui` 已落地 AG-UI event transfer、HTTP SSE event stream channel 和 action POST 回流第一片。生产级 adapter 负责把统一 `SurfaceMessage` 转成目标 payload、投递 payload、接收用户动作和维护目标平台 message id。用户动作必须通过 TurnLoop 的 input/resume/action 边界回到运行时，不能从 channel adapter 直接调用 graph/node。
...
不写完整实现时，最小实现顺序和当前状态如下：

1. `[done]` `graph.Run(ctx, state, opts...) iter.Seq2[Event, error]`
2. `[done: first slice]` `EventType` 已覆盖 run、turn、node、model route、tool、sandbox、memory、skill、MCP、A2A、checkpoint/import/resume/interrupt；checkpoint/interrupt 事件已和真实恢复流程绑定。
3. `[done]` root `NodeContext`、`NodeHandler` 和 `graph.WithNodeMiddleware`，采用 `c.Next()` 显式推进链路。
4. `[done: first slice]` root `EventContext`、`EventHandler`、Runner event middleware 和 `EventSinkMiddleware` strict/fallback 策略已存在。
5. `[done]` provider `ModelContext`/`ModelHandler` 和 tools `ToolContext`/`ToolHandler` 第一版。
6. `[done]` root `Runner` facade。
7. `[done: first slice]` root `TurnLoop` facade，包含 pending/resume queue、interrupted input 记录、默认 `TurnInputBatch`、`TurnInputMergeFunc` 业务级输入合并策略，以及内存、本地 file、row backend、filesystem blob backend 和通用对象 client blob backend `TurnLoopStore` 持久化第一片。
8. `[done: first slice]` `Plugin`、`PluginHost`、`PluginDescriptor`、`PluginCapability`、`AsyncEventSink`、node/event/model/tool middleware 注册、事件订阅、插件 subscriber strict/fallback、插件生命周期状态机、幂等 close、close-while-running 等待、Runner/TurnLoop close、trace HTTP/JSON exporter、OTLP/HTTP JSON exporter、LangSmith-compatible HTTP run exporter 和 LangGraph-style HTTP event exporter 已存在；真实 LangSmith SDK、LangGraph-style exporter 的 policy/redaction 深化仍需补齐。
