...
| agent scaffold generator | `cmd/gopact` | `go test -count=1 ./cmd/gopact` | `gopact agent init` emits a standalone A2A HTTP agent module; `gopact agent init-cluster` emits a local planner/worker/reviewer A2A HTTP cluster with registry, mesh bootstrap, routing, streaming, cancel, health, and shutdown tests; module paths default to example.com/<name>; generated clusters wire `GOPACT_A2A_REGISTRY_URL` into mesh bootstrap; generated agents and clusters use `GOPACT_A2A_REGISTRAR_URL` for optional external lease registration and heartbeat; `gopact agent verify` checks required scaffold files, bare-array A2A registry shape, `.gitignore` credential boundaries, and `go test ./...` without loading `.env`; `gopact agent verify` rejects scaffolds that do not ignore `.env`; `gopact agent run` executes a generated agent and loads local `.env` without overriding existing environment variables |
