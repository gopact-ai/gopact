...
| --- | --- | --- | --- |
| workflow graph execution | `graph` | `go test -count=1 ./graph ./gopacttest/graphconformance` | Typed graph runtime, node/edge execution, replay-planned and artifact-verified step export/import and checkpoint resume, interrupted step resume, branch routing, DAG fan-in, dynamic fan-out, explicit parallel fan-out with ordered merge, loop step limits, runnable subgraphs, node-emitted nested events, topology export, graph state and node schema guards, node middleware effect evidence, event stream, and reusable graph conformance |
...
| A2A agent mesh | `a2a` | `go test -count=1 ./a2a ./gopacttest/a2aconformance` | Agent card discovery, duplicate discovery coalescing, readiness-gated HTTP discovery, continuous sync refresh conformance, lease registration, heartbeat renewal, reusable discoverer/card registrar conformance, health-driven eviction, capability/tag routing, auth context, stable retry IDs, HTTP/JSON-RPC/SSE task transport |
...
| agent scaffold generator | `cmd/gopact` | `go test -count=1 ./cmd/gopact` | `gopact agent init` emits a standalone A2A HTTP agent module; `gopact agent init-cluster` emits a local A2A HTTP cluster with default planner/worker/reviewer agents or repeatable `-agent name[:capability[:description]]` domain-agent specs, registry, mesh bootstrap, routing, streaming, cancel, health, and shutdown tests; module paths default to example.com/<name>; generated clusters wire `GOPACT_A2A_REGISTRY_URL` into mesh bootstrap; generated agents and clusters use `GOPACT_A2A_REGISTRAR_URL` for optional external lease registration and heartbeat; `gopact agent verify` checks required scaffold files, bare-array A2A registry shape, `.gitignore` credential boundaries, `.env.example` standard agent or cluster environment variables, and `go test ./...` without loading `.env`; `gopact agent verify` rejects scaffolds that do not ignore `.env`; `gopact agent run` executes a generated agent and loads local `.env` without overriding existing environment variables |
