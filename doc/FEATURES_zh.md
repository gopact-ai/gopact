...
| A2A agent mesh | `a2a` | `go test -count=1 ./a2a ./gopacttest/a2aconformance` | Agent card discovery, duplicate discovery coalescing, readiness-gated HTTP discovery, lease registration, heartbeat renewal, health-driven eviction, capability/tag routing, auth context, stable retry IDs, HTTP/JSON-RPC/SSE task transport |
