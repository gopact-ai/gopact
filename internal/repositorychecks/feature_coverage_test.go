...
func TestFeatureCoverageMatrixDocumentsCoreCapabilities(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			boundary:   "replay-planned and artifact-verified step export/import and checkpoint resume, interrupted step resume, branch routing, DAG fan-in, dynamic fan-out, explicit parallel fan-out with ordered merge, loop step limits, runnable subgraphs, node-emitted nested events, topology export, graph state and node schema guards, node middleware effect evidence",
		},
		{
		...
		},
		{
			...
			boundary:   "duplicate discovery coalescing, readiness-gated HTTP discovery, continuous sync refresh conformance, lease registration, heartbeat renewal, reusable discoverer/card registrar conformance",
		},
		{
		...
		},
		{
			...
			boundary:   "repeatable `-agent name[:capability[:description]]` domain-agent specs, registry, mesh bootstrap, routing, streaming, cancel, health, and shutdown tests; module paths default to example.com/<name>; generated clusters wire `GOPACT_A2A_REGISTRY_URL` into mesh bootstrap; generated agents and clusters use `GOPACT_A2A_REGISTRAR_URL` for optional external lease registration and heartbeat; `gopact agent verify` checks required scaffold files, bare-array A2A registry shape, `.gitignore` credential boundaries",
		},
		{
