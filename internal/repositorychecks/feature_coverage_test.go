...
func TestFeatureCoverageMatrixDocumentsCoreCapabilities(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			boundary:   "step export/import, interrupted step resume, branch routing, DAG fan-in, dynamic fan-out, explicit parallel fan-out with ordered merge, loop step limits, runnable subgraphs, node-emitted nested events, topology export, graph state and node schema guards, node middleware effect evidence",
		},
		{
