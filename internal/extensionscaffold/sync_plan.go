...
func renderSyncScriptFile(plan SyncPlan) File {
	...
	b.WriteString("  echo \"tar is required to sync existing external repositories\" >&2\n")
	b.WriteString("  exit 127\n")
	b.WriteString("fi\n\n")
	...
	b.WriteString("}\n\n")
	b.WriteString("copy_generated_scaffold() {\n")
	b.WriteString("  local source_dir=\"$1\"\n")
	b.WriteString("  local target_dir=\"$2\"\n")
	b.WriteString("  (\n")
	b.WriteString("    cd \"${source_dir}\"\n")
	b.WriteString("    tar --exclude='.git' --exclude='.gocache' -cf - .\n")
	b.WriteString("  ) | (\n")
	b.WriteString("    cd \"${target_dir}\"\n")
	b.WriteString("    tar -xf -\n")
	b.WriteString("  )\n")
	b.WriteString("}\n\n")
	...
	b.WriteString("  local repo_exists=0\n")
	b.WriteString("  local sync_dir=\"\"\n")
	...
	b.WriteString("    repo_exists=1\n")
	b.WriteString("    sync_dir=\"$(mktemp -d)\"\n")
	b.WriteString("    gh repo clone \"${repo}\" \"${sync_dir}\"\n")
	b.WriteString("    copy_generated_scaffold \"${repo_dir}\" \"${sync_dir}\"\n")
	b.WriteString("    repo_dir=\"${sync_dir}\"\n")
	b.WriteString("  else\n")
	...
	b.WriteString("    git -C \"${repo_dir}\" commit -m \"chore: sync gopact extension scaffold\"\n")
	b.WriteString("  fi\n")
	b.WriteString("  if [[ \"${repo_exists}\" == \"1\" ]]; then\n")
