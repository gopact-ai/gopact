...
Use `docs/design/index.md` as the design map before making architectural or
public API changes.
...
- `adapters` contains reference-only or transitional adapters. Check
  `docs/design/repository-boundary.json` before adding, moving, or expanding an
  adapter.
...
- `docs/design` is the source of truth for module boundaries, public API policy,
...
- Preserve public API compatibility. Public root-package additions, removals, or
  semantic changes must update `docs/design/public-api-boundary.json` and, when
  applicable, `docs/design/public-api-examples.json`, examples, migration docs,
  and deprecation policy.
- Repository-boundary changes must update `docs/design/repository-boundary.json`,
